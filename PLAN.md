...
Where a rewrite lives. Not in `tutorials/`, which stays verbatim. A new folder, `lessons/` at the root, one page per rewritten lesson, using the front page's stylesheet, so the original and the rewrite can be read side by side and the front page can switch its link when the rewrite is better.
...
Every name in this family carries the `dew` prefix: dewlab, dewmini, dewmark, dewadaba. A few alternatives, in case dewadaba does not say enough.

- For the combined site: `dewsite`, `dewstack`. Both say "web" more than "database".
