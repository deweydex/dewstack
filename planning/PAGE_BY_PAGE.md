...
## 6. everlearning's `Teaching materials/`, copied to `sources/teaching-materials/`

_Added 2026-09-04, after Josh pushed a `Teaching materials/` folder to `everlearning`'s `main`. That folder holds material for several unrelated modules — maths worksheets, general programming notebooks, a matrices strand. Only the web authoring and database subset is copied here: `Databases/`, `Web Authoring Briefs/`, `Break and Make a Website/`, the two exam folders, and the two module descriptor PDFs. Nothing else moved. These are primary source material — a live, currently-taught sequence and a live, currently-assessed project brief — not another site to mine for prose, so this section reads differently from sections 1 to 4: it says what each piece is and where it changes the plan, not sentence counts._

### The Database Methods (5N0783) notebook sequence

| Item | What it is | Changes to the plan | Track | Do |
|---|---|---|---|---|
| `01_pandas_foundations.ipynb` through `05_pandas_to_sql.ipynb` | The real day-to-day teaching sequence: pandas first (DataFrames, filtering, sorting), then matplotlib (line, bar, histogram), then an "interactive reports" notebook built on ipywidgets, then file import/export (CSV, Excel), then SQLite and the pandas↔SQL bridge (`read_sql`, `to_sql`). | Data Arc 1 stays SQL-only, as decided; this sequence's shape belongs to Arc 2, which already assumed a real dataset, cleaning and a chart. It confirms pandas, matplotlib and an interactive element are graded, not optional, once Arc 2 starts. | Data | quarry for Arc 2; the five-notebook order is the reference for what each Arc 2 page needs to cover |
| `database_intro_sql_to_pandas.ipynb` | A library-books example that teaches every SQL clause (`SELECT`, `WHERE`, `ORDER BY`, `GROUP BY`, `JOIN`) beside its pandas equivalent, side by side. | A genuinely good device for a page that has to justify why the course teaches both. Worth keeping as the model for whichever Arc 2 page introduces `read_sql`/`to_sql`. | Data | quarry |
| `Database_Project_Brief_50-1 Final.pdf`, `Final_Database_Project_Brief_50.ipynb` | The live project brief, 50% of the module: a nine-section notebook built over several weeks against a student-chosen Our World in Data dataset — import via both `sqlite3`/`to_sql` and pandas, describe the fields, query, clean, chart with matplotlib/seaborn/plotly, export a report, build one ipywidgets interactive element, conclude, cite sources including any LLM use. | This is Data Arc 2, almost exactly as section 15 already sketched it, now with the graded shape confirmed: two import methods, cleaning as its own step, at least two charts, one interactive element (which stays a full-stack-arc idea here, per Josh's call, so Arc 2's version of "interactive" is the JS-form pattern deferred rather than literal ipywidgets), and a bibliography with LLM-use disclosure. | Data | the brief for Arc 2; write to it directly rather than reconstructing the shape from scratch |
| `DatabaseMethods5N0783 v2 Oct 2014.pdf` | The QQI module descriptor: learning outcomes 1 to 11, covering database concepts, query views, design with keys and relationships, and self-directed problem-solving. | Confirms LO9 to LO11 (design, build, improve) are the multi-table half of Arc 2 that the single-dataset project brief alone doesn't cover — that half is what the exam material below is for. | Data | reference only |
| `Database_Practice_Exam/`, `Database Exam Test/`, `DatabaseExam/` | Several past 2-hour practical exams, each a themed multi-table relational database (Unseen University, a wizards' registry, Lord of the Rings, HVIT), with reference sheets and, for one, a bugfixed marking sheet. | Coverage material for the multi-table, keys-and-relationships half of Arc 2 (design before typing, a join). Not a source for prose; a source for realistic exam-shaped practice questions once Arc 2 exists. | Data | keep as coverage; mine for practice-page questions once Arc 2 is written |
| `uu_reference.md` | A one-page exam reference sheet: `sqlite3`, SQL clauses, pandas, ipywidgets, matplotlib, each with one worked example. | The shape a "quick reference, SQL half" page should take — compact, one worked example per row, not prose. | Both | model for the quick reference's SQL half |

### The Web Authoring (5N1910) briefs

| Item | What it is | Changes to the plan | Track | Do |
|---|---|---|---|---|
| `WA_Project_Brief_Streamlined.md` | The live, currently-assessed 70% project brief for the second starter: five pages, Flexbox and Grid both required, a fixed marks table, a phased "plan, build, test and document" approach. | Confirms section 15's Arc 2 shape (Flexbox and Grid both mandatory) and gives the file list to build the second starter against directly, rather than inferring it. | Web | the brief for the second starter |
| `files/Project_Planning_Template.md`, `Project_Readme_Template.md` | Templates for `planning.md` and `readme.md`, written for one past assignment topic (documenting four ML projects across five pages) rather than generic. | The questions and structure are reusable; the topic-specific parts (page names like `neuron.html`) are not. A dewstack version keeps the shape and genericises the topic. | Web | adapt, do not copy verbatim |
| `files/Assignment1_Planning_Template.md`, `Assignment1_Readme_Template.md` | The equivalent pair for the first assignment (a single landing page), generic rather than topic-specific. | Closer to what the starter (`deweydex/web`) needs for its own planning/readme gap (section 5, step 5). | Web door | adapt for the starter's planning/readme gap |
| The brief's required-files list | `planning.md`, `readme.md`, `maintenance.md` | The second starter's file list gains `maintenance.md`, which section 15 hadn't listed. | Web | add to the second starter's skeleton |
| `WA_Assignment1_Level5.pdf`, `WA_Assignment1_Portfolio.docx`, `WA_Project_Brief.docx`, `WebAuthoring5N1910 v2 2013.pdf` | The Level 5 assignment brief and module descriptor. | Cross-checked against `deweydex/web`'s exercises and section 5's gap analysis; no surprises. | Web door | reference only |

### Break and Make a Website

The folder holds two unrelated things under one name, easy to conflate: the
Irish Tax Simulator at its top level, and a full teacher's kit for the
`HTML-CSS-SQL-JS` playground nested inside `Break and Make a Website
copy/files/` and its exact duplicate `files2/`.

| Item | What it is | Changes to the plan | Track | Do |
|---|---|---|---|---|
| `index.html`, `styles.css`, `script.js`, `README.pdf`, `QUICK_START.pdf`, `Teachers Guide/` (`LESSON_PLAN.md`, `ANSWER_KEY.md`, `PEDAGOGICAL_NOTES.md`) | The Irish Tax Simulator: sliders, Chart.js charts, dark mode, `localStorage`, a graded beginner/intermediate/advanced exercise ladder, a debugging-tips section, and its own 8-to-10-session lesson plan and answer key. | Neither web arc currently touches JavaScript. Decided 2026-09-04: the artefact itself is a fun enrichment resource, kept outside both arcs; the "break something, watch it, then fix it" technique it uses is being considered on its own for a Level 6 track. See `planning/LEVEL6_COVERAGE.md`. | Web, enrichment | the artefact: coverage material for an enrichment page, not scheduled; the technique: candidate for Level 6 |
| `files/index.html`, `styles.css`, `tutorial.js`, `teacher.html` | Byte-identical to `sources/playground/` apart from the course bar `add_course_bar.py` already inserts there — the same `HTML-CSS-SQL-JS` playground, not new content. | None; already assessed in section 3 above. | Data | nothing new; the playground assessment in section 3 stands |
| `files/LESSON_PLAN.md`, `PEDAGOGICAL_NOTES.md`, `FILE_MANIFEST.md`, `BONUS_EXERCISES.md`, `DEPLOYMENT_GUIDE.md`, `TROUBLESHOOTING_STUDENTS.md`, `TROUBLESHOOTING_TEACHERS.md` | What section 3 flagged as missing when it assessed `teacher.html`: a genuine teacher's kit for the same playground — lesson plans at five class-length formats (1-hour workshop through full semester), an assessment rubric, a common-errors guide split by audience, ten bonus exercises. Not in `HTML-CSS-SQL-JS` itself. | Fills section 3's fold instruction ("notes are not student-facing; they belong in `planning/`") with real material instead of notes reconstructed from `teacher.html`. Worth quarrying when the plushies quiz and the playground's SQL section are rewritten (`PAGE_BY_PAGE.md` section 5, item 3). | Data | quarry for `planning/TEACHING_NOTES.md` when that step arrives |
| `files/tutorial.ts`, `TYPESCRIPT_GUIDE.md` | A TypeScript-annotated variant of the same tutorial, with a guide explaining TypeScript concepts to a reader with no prior programming. | dewstack doesn't teach TypeScript and has no reason to; not a fit anywhere in the current plan. | none | drop |

### What this doesn't change

Data Arc 1 stays SQL-only (Josh's call, 2026-09-04): pandas is not pulled forward from Arc 2, even though the real course teaches it first. The ipywidgets-style interactive element stays a full-stack-arc idea rather than moving into Arc 2, so Arc 2's "interactive" requirement is met by a chart and the JS-form-calls-Python pattern section 12 already describes, not by literal ipywidgets, which cannot run outside a live Jupyter kernel anyway.
