...
| Web Arc 2, D1 to D4 (planning a site; pages and navigation; cards in a row; a grid gallery) | `project_wad`'s own files; `sources/wadb/lessons/11-flexbox-grid-advanced.html` for flex-grow/shrink/basis and `minmax()` | `tutorials/web/`, module `web`, new series `several-pages` | done 2026-09-05; `NEXT_STEPS.md` step 6, in progress; each "your turn" points at `project_wad`'s real code, not new code written for the tutorial |
